...
const PayrollDashboardForAllEmployee = () => {
  ...
  const fetchSalaryRecords = async () => {
    try {
      ...
      console.log(data.salaryRecords);
  ...
  const fetchSpecialEvents = async () => {
    try {
      ...
      const response = await axios.get(`${APIDictionary.events}`, { withCredentials: true });
      if (response.data.success) {
        ...
        const payrollEvents = response.data.data.filter(
          (event: EventData) => event.type === 'BILL' || 
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="space-y-6">
        <SpecialEvents events={specialEvents} isLoading={isLoadingEvents} />
      </div>
    </div>
  );
};
